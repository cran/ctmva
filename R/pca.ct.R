...
#' @export pca.ct
pca.ct <-
  function(fdobj, cor=FALSE, common_trend=FALSE) {
    ...
    output <- eigen(cmat)
    if(!is.null(fdobj$fdnames$reps)) {
      rownames(output$vectors) <- paste0(fdobj$fdnames$reps)
      colnames(output$vectors) <- paste0(fdobj$fdnames$reps)
    }
    else {
      rownames(output$vectors) <- paste0("Comp.", 1:ncol(fdobj$coef))
      colnames(output$vectors) <- paste0("Comp.", 1:ncol(fdobj$coef))
    }
    results <- list()
    results$var <- output$values
    results$loadings <- output$vectors
    # add scores
    ...
    coefmat <- if (cor) fdobj$coefs %*% isr else fdobj$coefs
    scoef <- (coefmat - w %*% crossprod(phibar, coefmat)) %*% output$vectors
    results$scorefd <- fd(scoef, fdobj$basis)
    return(results)
  }
